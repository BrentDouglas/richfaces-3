...
A4J.AJAX.innerXML = function(/*Node*/node){
    // summary:
    //        Implementation of MS's innerXML function. Straight from dojox
    // node:
...
A4J.AJAX.XMLHttpRequest.prototype = {
    ...
    updatePagePart : function(id, isLast){
        ...
        if ( oldnode  ) {
            ...
            if(!window.opera && !A4J.AJAX.isWebkitBreakingAmps() && oldnode.outerHTML && !oldnode.tagName.match( /(tbody|thead|tfoot|tr|th|td)/i ) ){
                   ...
                   oldnode.outerHTML = A4J.AJAX.innerXML(newnode);
            } else {
                ...
                if (!A4J.AJAX._testReplacedGetElementByIdVisibility) {
                    ...
                    document.getElementById = function(id) {
                        ...
                        if (!elt) {
                            ...
                            try {
                                var result = importednode.ownerDocument.evaluate(
                                    "//*[@id='" + id + "']",
                                    importednode,
                                    null,
    ...
    appendNewHeadElements : function(callback){
        ...
        var _this = this;
        includes.concat(
            this._appendNewElements("link","href","class",
                ["component","user"],
                ["type","rev","media"],
                {"class": "className"},
    ...
    if ((!document.all || window.opera) && !A4J.AJAX._scriptTested){
        try{
            ...
            if (!window.opera && !A4J.AJAX.isWebkitBreakingAmps() && _span.outerHTML) {
                _span.outerHTML = A4J.AJAX.innerXML(_script);
            } else {
                var importednode ;
            ...
            /* Mozilla in XHTML mode not have innerHTML */
        }
    }
